...
if size(rVals)>0
    ...
    T = single(p.T_keep(kp,:));
    C = single(p.C_keep(kp,:));
    Y = p.Ym;
    A = p.A_keep(:,kp);
    K = size(T,1);
    Kms(1) = K;
    ...
    fff = p.f;
    bbb = p.b;
    T = single(NaN(1,size(fff,2)));
    C = single(NaN(1,size(fff,2)));
    Y = p.Ym;
    A = NaN(size(bbb,1),1);
    K = 1;
    Kms(1) = K;
...
save([path 'collated_caiman_output_minSNR_0p01.mat'],'T_all','N_all','C_all','offsets','-v7.3')
