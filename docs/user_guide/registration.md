...
Just like {ref}`assembly outputs <extraction_inputs>`, registration outputs to {code}`.h5` format.

Registration outputs have the following groups:

{code}`/Y`
: Takes the name of the {ref}`ds <ds>` parameter. This group contains the 2D timeseries.
