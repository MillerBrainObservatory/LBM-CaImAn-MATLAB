classdef Photostim < scanimage.interfaces.Component & most.HasMachineDataFile
    ...
    methods
        ...
        %       photostimObj.start()
            ...
            if obj.zMode3D
                imagingZid = obj.hSI.hFastZ.zScannerId(obj.hScan2D.name);
            ...
            try
                ...
                obj.hSI.hShutters.shuttersTransition(obj.hScan.mdfData.shutterIDs, true);
        ...
        %       photostimObj.triggerSync()
            ...
            if ~obj.hSI.mdfData.simulated && ~isempty(obj.syncTriggerTerm)
        ...
        %       photostimObj.abort()            
            ...
            if ~isempty(obj.hScan)
                obj.hSI.hShutters.shuttersTransition(obj.hScan.mdfData.shutterIDs, false);
            end
    ...
    %% INTERNAL METHODS
    methods (Hidden)
        function compensateMotion(obj,motionMatrix)
            if obj.active && obj.compensateMotionEnabled
                if ~isequal(obj.lastMotion,motionMatrix)
                    obj.lastMotion = motionMatrix;
                    
                    fixPt_Ref = [0 0];
                    fixPt_LinScan = scanimage.mroi.util.xformPoints(fixPt_Ref,obj.hScan.scannerToRefTransform,true);
                    
                    motionPt_Ref = scanimage.mroi.util.xformPoints([0 0 0],motionMatrix,true);
                    motionPt_LinScan = scanimage.mroi.util.xformPoints(motionPt_Ref(1:2),obj.hScan.scannerToRefTransform,true);
                    
                    offsetAngleXY = motionPt_LinScan - fixPt_LinScan;
        ...
        function [ao, samplesPerTrigger, path] = generateAO(obj)
            ...
            switch obj.stimulusMode
                case 'sequence'
                    ...
                    if obj.hasGalvos
                        assert(min(cellfun(@(x)size(x.G,1), AOs)) > 0, 'One or more stimulus groups in the sequence were empty. Remove from the sequence to avoid unexpected results.');
                        
                        %pad AOs
                        [samplesPerTrigger.G, i] = max(cellfun(@(x)size(x.G,1), AOs));
                        ...
                        [~, i] = max(cellfun(@(x)size(x.B,1), AOs));
                        assert(min(cellfun(@(x)size(x.B,1), AOs)) > 0, 'One or more stimulus groups in the sequence were empty. Remove from the sequence to avoid unexpected results.');
                    end
                    
                    %pad AOs
                    if obj.hasBeams
                        samplesPerTrigger.B = size(AOs{i}.B, 1);
                    ...
                    %pad AOs
                    if obj.hasZ
                        samplesPerTrigger.Z = size(AOs{i}.Z, 1);
                ...
                case 'onDemand'
                    ...
                    if obj.hasGalvos && obj.hasBeams
                        samplesPerTrigger = arrayfun(@(x)struct('G', size(x.G,1), 'B', size(x.B,1)),ao);
                    elseif obj.hasGalvos
                        samplesPerTrigger = arrayfun(@(x)struct('G', size(x.G,1)),ao);
                    else
                        samplesPerTrigger = arrayfun(@(x)struct('B', size(x.B,1)),ao);
                    end
                    if obj.hasZ
    ...
    methods (Hidden)
        ...
        function startLogging(obj)
            if obj.logging && obj.hSI.hChannels.loggingEnable
                ...
                obj.startMonitoring(true); %start logging
            end
        end
