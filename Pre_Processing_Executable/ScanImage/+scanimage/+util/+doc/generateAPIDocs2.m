...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if nargin < 1 || isempty(dir2Doc)
        [~, name, ~] = fileparts(pwd);
        rightDir = strcmp(name, 'ScanImage');
        if(~rightDir)
    ...
    xFileExists = exist('C:\bitbucket\ScanImage\+scanimage\+util\+doc\DocGenExcludedDIRS.txt', 'file');
    if(xFileExists)
        fileId = fopen('C:\bitbucket\ScanImage\+scanimage\+util\+doc\DocGenExcludedDIRS.txt');
        excludedDirs = textscan(fileId, '%s','Delimiter', ',');
        fclose(fileId);
    else
        error('The exclusion file DocGenExcludedDIRS.txt does not exist in directory C:\bitbucket\ScanImage\+scanimage\+util\+doc');
    end
