...
%     end
    ...
    for ijk = 1
        ...
        savefast(fullfile(tmpPath, [fileName(1:end-3) 'mat']),'vol','volumeRate','pixelResolution','fullVolumeSize');
        tt = toc/3600;
    ...
    for aaa = 8:numberOfPlanes
        ...
        for ddd = 1:numFiles
            if multiFile
                if ddd < 10
                    matfilename = fullfile(tmpPath, [fileNameRoot '_0000' num2str(ddd) '.mat']);
                else
                    matfilename = fullfile(tmpPath, [fileNameRoot '_000' num2str(ddd) '.mat']);
                end
            else
                matfilename = fullfile(tmpPath, [fileNameRoot '.mat']);
            end
    ...
    %     [~,minv_idx] = mink(shifts_v,120,1);
        ...
        outputFile = fullfile(tmpPath, [fileNameRoot '_plane_' num2str(aaa) '.mat']);
    ...
    for xyz = 1:numFiles
        ...
        delete(fullfile(tmpPath, [fileName(1:end-3) 'mat']));
