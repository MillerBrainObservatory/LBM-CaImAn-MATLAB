function [cY,mY,ng] = motion_metrics_jeff(Y,bnd,batch_size,var_name)
...
%         y_temp = single(load_data(Y,t,batch_size));
        ...
        if strcmp(var_name,'Y')
            y_temp = single(Y.Y(:,:,:,t:t+lY-1));
        else
            y_temp = single(Y.M(:,:,:,t:t+lY-1));
        end
...
elseif ndims(Y)-1 == 3;
    ...
    ng = 0; % added this because we don't use ng and don't have access to Y without a memmapped rigamorole
end
