...
p = load(fullfile(path, 'caiman_output_plane_1.mat')); % Load stuff from first p
rVals = p.rVals;
...
c = load(fullfile(path, 'three_neuron_mean_offsets.mat'),'offsets');
...
for ijk = 2:30
    ...
    pm = load(fullfile(path,[ 'caiman_output_plane_' num2str(ijk) '.mat']));
...
save(fullfile(path, 'collated_caiman_outut_minSNR_1p4.mat'),'T_all','N_all','C_all','offsets','-v7.3')
