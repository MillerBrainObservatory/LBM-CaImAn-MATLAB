...
%% additional parameters for assembly
...
for plane_idx = 1:num_planes
    ...
    if do_figures
        ...
        labels = {'Random Frame', 'Mean Image'};
        scale_full = calculate_scale(size(img_frame, 2), metadata.pixel_resolution);
        scales = {scale_full, scale_full};
        plane_save_path = fullfile(fig_save_path, sprintf('mean_image_plane_%d.png', plane_idx));
    ...
    metadata.data_size = size(z_timeseries);
...
if offset ~= 0
    dataOut = dataOut(:, 1+floor(abs(offset)/2):end-floor(abs(offset)/2), :, :);
end
end
