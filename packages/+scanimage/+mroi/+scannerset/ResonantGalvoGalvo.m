            ...
            switch upper(scanner)
                case 'G'
                    ...
                    if ~isempty(obj.scanners{2})
                        ...
                        obj.scanners{2}.clearCachedWaveform(sampleRateHz, ao_volts(:,1));
                    end
                    
                    if nargin < 4 || isempty(sampleRateHz)
                        sampleRateHz = obj.scanners{3}.sampleRateHz;
                    end
                    obj.scanners{3}.clearCachedWaveform(sampleRateHz, ao_volts(:,2));
                case 'Z'
                    if nargin < 4 || isempty(sampleRateHz)
        ...
        function [ao_volts_out,metaData] = retrieveOptimizedAO(obj, scanner, ao_volts, sampleRateHz) 
            ...
            switch upper(scanner)
                case 'G'
                    ...
                    [metaData,ao_volts_out] = obj.scanners{3}.getCachedOptimizedWaveform(sampleRateHz,ao_volts(:,2));
                    if ~isempty(obj.scanners{2})
                        [metaData_1,ao_volts_temp_1] = obj.scanners{2}.getCachedOptimizedWaveform(sampleRateHz,ao_volts(:,1));
                        
                        if ~isempty(metaData_1)
                            ao_volts_out = horzcat(ao_volts_temp_1,ao_volts_out);
                            metaData(end+1) = metaData_1;
                        end
                    end
                case 'Z'
                    if nargin < 4 || isempty(sampleRateHz)
        ...
        function ao_volts = optimizeAO(obj,scanner,ao_volts,sampleRateHz)            
            switch upper(scanner)
                case 'G'
                    ...
                    ao_volts(:,2) = obj.scanners{3}.optimizeWaveformIteratively(ao_volts(:,2),rate);
                case 'Z'
                    if nargin < 4 || isempty(sampleRateHz)
