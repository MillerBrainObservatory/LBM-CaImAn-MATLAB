classdef Acquisition < scanimage.interfaces.Class
    ...
    properties (Hidden, Constant)
        ...
        ADAPTER_MODULE_CHANNEL_COUNT = containers.Map({'NI5732','NI5733','NI5734','NI5751','NI517x','NI5771','NI5772'},{2,2,4,4,4,4,16});
        ADAPTER_MODULE_RAW_SAMPLING_RATE_MAP = containers.Map({'NI5732','NI5733','NI5734','NI5751','NI517x','NI5771','NI5772'},{80e6,120e6,120e6,50e6,250e6,1.5e9,.8e9});
        FPGA_STATE_MACHINE_LOOP_RATE = containers.Map({'NI5732','NI5733','NI5734','NI5751','NI517x','NI5771','NI5772'},{[],[],[],[],[],100e6,100e6});
        FPGA_RAW_ACQ_LOOP_ITERATIONS_COUNT_FACTOR = containers.Map({'NI5732','NI5733','NI5734','NI5751','NI517x','NI5771','NI5772'},{1,1,1,1,2,8,4});
        FPGA_RAW_SAMPLE_RATE_TO_SAMPLE_RATE_FACTOR = containers.Map({'NI5732','NI5733','NI5734','NI5751','NI517x','NI5771','NI5772'},{1,1,1,1,2,[],[]});
        ADAPTER_MODULE_SAMPLING_RATE_RANGE_MAP = containers.Map({'NI5732','NI5733','NI5734','NI5751','NI517x','NI5771','NI5772'},{[20e6 80e6],[50e6 120e6],[50e6 120e6],[50e6 50e6],[20e6 125e6],[850e6 1.5e9],[400e6 800e9]});
        ADAPTER_MODULE_TRIGGER_TO_ADC_DELAY = containers.Map({'NI5732','NI5733','NI5734','NI5751','NI517x','NI5771','NI5772'},{16,16,16,0,0,0,0}); % TODO: Evaluate trigger delay for NI5751 and NI517x
        ADAPTER_MODULE_ADC_BIT_DEPTH = containers.Map({'NI5732','NI5733','NI5734','NI5751','NI517x','NI5771','NI5772'},{14,16,16,14,14,16,16});
        ADAPTER_MODULE_AVAIL_INPUT_RANGES = containers.Map({'NI5732','NI5733','NI5734','NI5751','NI517x','NI5771','NI5772'},{[1,0.5,0.25],[1,0.5,0.25],[1,0.5,0.25],[1,0.5,0.25],[2.5,1,0.5,0.1],[1],[1]});
    ...
    methods
        ...
        function set.rawSampleRateAcq(obj,val)
            ...
            rawSampleRateToSampleRate = obj.FPGA_RAW_SAMPLE_RATE_TO_SAMPLE_RATE_FACTOR(obj.flexRioAdapterModuleName);
            if ~isempty(rawSampleRateToSampleRate)
                obj.sampleRateAcq = obj.rawSampleRateAcq ./ rawSampleRateToSampleRate;
            end
            
            stateMachineLoopRate = obj.FPGA_STATE_MACHINE_LOOP_RATE(obj.flexRioAdapterModuleName);
...
%                 stripeData.overvoltage = dabs.ni.oscope.checkOverload;
        ...
        function configureAdapterModuleSampleClock(obj,mode)
            ...
            function configureExternalSampleClock()
                ...
                loopRateRange = obj.ADAPTER_MODULE_SAMPLING_RATE_RANGE_MAP(obj.flexRioAdapterModuleName);
        ...
        function measureExternalRawSampleClockRate(obj)
            ...
            loopRateRange   = obj.ADAPTER_MODULE_SAMPLING_RATE_RANGE_MAP(obj.flexRioAdapterModuleName);
            maxLoopRateStd  = 30e3;    % [Hz] TODO: need to fine tune this value (somewhat of a guess right now)
            ...
            loopIterationsCountFactor = obj.FPGA_RAW_ACQ_LOOP_ITERATIONS_COUNT_FACTOR(obj.flexRioAdapterModuleName);
        ...
        function NI577xMeasureSampleClockRate(obj)
            measurePeriod     = 1e-3;   % [s] count sample clock edges for measurePeriod of time
            numMeasurements   = 100;    % number of measurement repeats to calculate mean and standard deviation
            maxLoopRateStd  = 3e3;    % [Hz] TODO: need to fine tune this value (somewhat of a guess right now)
            ...
            assert(obj.sampleRateAcq < obj.FPGA_STATE_MACHINE_LOOP_RATE(obj.flexRioAdapterModuleName),...
                'Error: The NI5771 trigger rate (%.3fMHz)is greater than the processing loop rate (%.3fMHz). This will result in lost data points!',...
                obj.sampleRateAcq/1e6,obj.FPGA_STATE_MACHINE_LOOP_RATE(obj.flexRioAdapterModuleName)/1e6);
