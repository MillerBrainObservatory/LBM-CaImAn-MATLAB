        ...
        function obj = Acquisition(hSlmScan)
            ...
            if strncmp(dev, 'RIO', 3) && ~isnan(str2double(dev(4:end)))
                ...
                if obj.hSlmScan.hSI.fpgaMap.isKey(dev)
                    ...
                    hFpga = dabs.ni.rio.NiFPGA(pathToBitfile,obj.hSlmScan.simulated);
    ...
    methods (Hidden)
        ...
        function acquisitionLoop(obj)
            ...
            while true
                ...
                [I,J] = ind2sub(size(sfInfo.buffer),obj.counters.currentSfSample);
                ...
                if obj.counters.currentSfSample == sfInfo.totalSamples
