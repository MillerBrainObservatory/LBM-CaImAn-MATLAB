classdef RoiGroupEditor < most.Gui & most.HasClassDataFile
    ...
    methods
        function obj = RoiGroupEditor(hModel, hController)
            ...
            obj.h2DImagingPlaneLines = line(nan,nan,'color','w','parent',obj.h2DProjectionViewAxes,'linewidth',1.5,'ButtonDownFcn',@obj.zScroll);
            ...
            obj.h2DScannerFovSurf = surface([0 1], [0 1], ones(2),'FaceColor','texturemap','edgecolor','y','linewidth',.5,'linestyle',':','parent',obj.h2DMainViewAxes,'hittest','off','ButtonDownFcn',@obj.fovSurfHit,'FaceAlpha',0.5);
            ...
            obj.hSnapLineX = line([0,0],[-999999 999999],2*ones(1,2),'parent',obj.h2DMainViewAxes,'color','c','linestyle',':','visible','off','linewidth',1);
            obj.hSnapLineY = line([-999999 999999],[0,0],2*ones(1,2),'parent',obj.h2DMainViewAxes,'color','c','linestyle',':','visible','off','linewidth',1);
            obj.hSnapLineR = line([0 0],[0,0],2*ones(1,2),'parent',obj.h2DMainViewAxes,'color','c','linestyle',':','visible','off','linewidth',1);
        ...
        function set.editorMode(obj,v)
            ...
            switch v
                case 'imaging'
                    ...
                    obj.canDrawArray = isa(obj.scannerSet,'scanimage.mroi.scannerset.ResonantGalvoGalvo') && ~obj.scannerSet.resonantLimitedFovMode;
...
%             end
            ...
            ylabel(obj.h2DProjectionViewTickAxes,[prefix 'm']);
                    ...
                    if obj.optimizeTransitions
                        ...
                        for id = setdiff(1:numel(obj.editingGroup.rois),transIds)
                            ...
                            elseif sf.isPark
                                ...
                                [stimPts,~] = obj.scannerSet.scanPathStimulusFOV(sf,0,roi.zs,0,true,false);
                                if isfield(stimPts,'Z')
            ...
            if ~isempty(i)
                ...
                elseif obj.editorModeIsStim
                    ...
                    switch i
                        ...
                        case 7
                            % z column
                            if ~isnan(evt.NewData) && isa(obj.tblMapping{j,1},'scanimage.mroi.scanfield.ScanField')
...
%                 obj.selectedObj.centerXY(obj.projectionDim) = nwctr;
        ...
        function [inds, rs] = findBrushAffectedInds(obj,pt)
            ...
            persistent boundsx
            persistent boundsy
            persistent supportsVecnorm
            
            ss = obj.scannerSet;
            if isa(ss,'scanimage.mroi.scannerset.GalvoGalvo')
                ss = ss.slm;
            end
            T = ss.refToScannerTransform;
            T(7:8) = 0;
            T2 = ss.scanners{1}.geometryBuffer.meterToPixelTransform;
            TT_ = T2*T;
            
            % buffer xx_mesh,yy_mesh in reference space for performance
            if isempty(res) || ~isequal(res,ss.scanners{1}.pixelResolutionXY) || ~isequal(TT,TT_)
                res = ss.scanners{1}.pixelResolutionXY;
                TT = TT_;
                [xx_mesh,yy_mesh] = meshgrid(1:res(1),1:res(2));
                [xx_mesh,yy_mesh] = scanimage.mroi.util.xformMesh(xx_mesh,yy_mesh,TT,true);
                mesh = [xx_mesh(:),yy_mesh(:)];
                boundsx = [min(mesh(:,1)) max(mesh(:,1))];
                boundsy = [min(mesh(:,2)) max(mesh(:,2))];
                supportsVecnorm = ~verLessThan('matlab','9.3.0'); %vecnorm is supported starting Matlab 2017b
            end
            
            % determine brush size in reference space
            pt1 = [0 0];
            pt2 = [obj.slmBitmapBrushSize 0];
            pt1 = scanimage.mroi.util.xformPoints(pt1,TT,true);
            pt2 = scanimage.mroi.util.xformPoints(pt2,TT,true);
            brushSize = norm(pt2-pt1);
            
            outOfBounds = (pt(1)+brushSize/2)<boundsx(1) || (pt(1)-brushSize/2)>boundsx(2) || ...
                          (pt(2)+brushSize/2)<boundsy(1) || (pt(2)-brushSize/2)>boundsy(2);
            ...
            mesh_pt = bsxfun(@minus,mesh,pt);
            if supportsVecnorm
                rs_squared = vecnorm(mesh_pt,2,2);
            else
                mesh_pt = mesh_pt.^2;
        ...
        function draw3D(obj,rois)
            ...
            if ~isempty(obj.scanPathCache) && isfield(obj.scanPathCache, 'Z')
...
%             obj.zProjectionRange = obj.zProjectionRange;
        ...
        function stimRoiPropsPanelCtlCb(obj,src,~)
            ...
            switch(src.Tag)
                ...
                case 'pmFunction'
                    fn = obj.stimFcnOptions{src.Value};
...
%                 end
        ...
        function frameAcquired(obj,updateToZ)
            ...
            if obj.showLiveImage && obj.Visible &&strcmp(obj.hModel.hRoiManager.scanType, 'frame')
                if obj.liveImageChannel
...
%                         [~,~,~,~,rot,~] = scanimage.mroi.util.paramsFromTransform(cll.aff);
        ...
        function loadBitmapFromFile(obj,varargin)
            ...
            if (numel(nwSz) ~= 2) || ~all(nwSz == fliplr(ss.scanners{1}.pixelResolutionXY))
