classdef ConfigurationEditor < most.Gui 
    ...
    methods
        ...
        function constructAddlCompsPnl(obj)
            if ~most.idioms.isValidObj(obj.hAdditionalComponentsPanel)
                ...
                uicontrol(args{:},'string', 'Add Cameras','position',[46 ph-196 300 50],'callback',@(varargin)addComp('wizardAddedCameras','CameraManager'));
    ...
    methods
        ...
        function initMotorSlmInfo(obj)
            ...
            obj.hSlmRegistry = scanimage.components.scan2d.slmscan.SlmRegistry.slmMap;
            ks = obj.hSlmRegistry.keys;
            for i = 1:numel(ks)
                dn = obj.hSlmRegistry(ks{i}).DisplayName;
                obj.slmName2RegMap(dn) = ks{i};
            end
        end
        ...
        function refreshPages(obj,varargin)
            ...
            if isempty(remHdgs)
                ...
                obj.hCamerasPage = scanimage.guis.configuration.CamerasPage(obj, true);
            ...
            [obj.hCamerasPage, camerasIdx] = isPageThere('CameraManager', obj.hCamerasPage,...
                @scanimage.guis.configuration.CamerasPage);
            ...
            removeIdx = removeIdx | camerasIdx | motorsIdx | fastzIdx | stimIdx |...
                integrationIdx | bscope2Idx | ecuIdx | pmtIdx | lscIdx;
            
            % MM Cam pages should already be created, this fucntion
            % (refresh) called after creating MM Pages. Make sure MM pages
            % are not created again.
            if isempty(obj.hCamerasPage)
                MMDat = {};
            else
                MMDat = obj.hCamerasPage.hCamerasTable.Data(:,2);
                MMDat = MMDat(~cellfun('isempty', MMDat));
            end
            
            [obj.hMMCamerasPage, mmHeadingIdx] = isNamedPageThere(...
                'MicroManager Camera', obj.hMMCamerasPage,...
                @scanimage.guis.configuration.MMCamerasPage, MMDat);
            
            [obj.hSimCamerasPage, simHeadingIdx] = isNamedPageThere(...
                'Simulated Camera', obj.hSimCamerasPage,...
                @scanimage.guis.configuration.SimCamerasPage, MMDat);
            [obj.hSpinCamerasPage, spinHeadingIdx] = isNamedPageThere(...
                'Spinnaker Camera', obj.hSpinCamerasPage,...
                @scanimage.guis.configuration.SpinCamerasPage, MMDat);
            
            removeIdx = removeIdx | mmHeadingIdx | simHeadingIdx | spinHeadingIdx;
            ...
            obj.hAllPages = [obj.hSIPage, obj.hShuttersPage, obj.hBeamsPage,...
                ...
                obj.hLSCPureAnalogPage, obj.hCamerasPage, obj.hMMCamerasPage{:},...
                obj.hSimCamerasPage{:}, obj.hSpinCamerasPage{:}, obj.hOtherPages{:}];
            if obj.isWizardMode
                ...
                if obj.wizardAddedCameras
                    cameraPages = [{obj.hCamerasPage} obj.hMMCamerasPage obj.hSimCamerasPage obj.hSpinCamerasPage];
                    extraComps = [extraComps cameraPages];
                end
        ...
        function pbApplyCb(obj,varargin)
            ...
            switch obj.hActivePage
                ...
                case obj.hCamerasPage
                    %populate cameraTypes based on expected names
                    mmCamsIdx = strcmpi(s.cameraTypes, 'micromanager');
                    simCamsIdx = strcmpi(s.cameraTypes, 'simulated') |...
                        strcmpi(s.cameraTypes, 'sim');
                    spinCamsIdx = strcmpi(s.cameraTypes, 'spinnaker');
                    
                    mmNames = s.cameraNames(mmCamsIdx);
                    simNames = s.cameraNames(simCamsIdx);
                    spinNames = s.cameraNames(spinCamsIdx);
                    
                    %reload/instantiate all camera types
                    [obj.hMMCamerasPage, mmRefReq] = reloadDepPages(obj.hMMCamerasPage,...
                        mmNames,@scanimage.guis.configuration.MMCamerasPage);
                    [obj.hSimCamerasPage, simRefReq] = reloadDepPages(obj.hSimCamerasPage,...
                        simNames,@scanimage.guis.configuration.SimCamerasPage);
                    [obj.hSpinCamerasPage, spinRefReq] = reloadDepPages(obj.hSpinCamerasPage,...
                        spinNames, @scanimage.guis.configuration.SpinCamerasPage);
                    refreshRequired = refreshRequired || mmRefReq || simRefReq ||...
                        spinRefReq;
            end
            if obj.isWizardMode
