clc;
% filename = fullfile('C:/Users/RBO/Documents/data/high_res/raw/MH70_0p6mm_FOV_50_550um_depth_som_stim_199mW_3min_M1_00001_00001.tif');
...
totalFrame = size(roiData{1}.imageData{1}{1}{1}, 2); % total number of frames in data set
...
for channel = 1:totalChannel
    ...
    for strip = 1:numROIs
        ...
        for frame = 1:totalFrame
            stripTemp = cat(4, ...
                stripTemp, ...
                single(roiData{1,strip}.imageData{1,channel}{1}{1}{frame}) ...
                );
        end
...
figure; subplot(1,2,1); imagesc(mean_image);
axis image; axis tight; axis off; colormap gray;
% Define the zoomed-in area (same as xlim and ylim of the second graph)
x_start = 247.3;
x_end = 315.6;
y_start = 287.8;
y_end = 363.2;

% Draw a red rectangle on the first subplot to represent the zoomed-in area
hold on;
rectangle('Position', [x_start, y_start, x_end - x_start, y_end - y_start], ...
          'EdgeColor', 'r', 'LineWidth', 2);
hold off;

subplot(1,2,2);imagesc(mean_image);
axis image; axis tight; axis off; colormap gray;
xlim([247.3 315.6])
ylim([287.8 363.2])
