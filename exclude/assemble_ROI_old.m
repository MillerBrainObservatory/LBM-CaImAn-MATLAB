function [imageData,frameRate,pixelResolution] = assemble_ROI_old(filename, chans)
if ~exist('chans','var'); chans = 1; end
...
totalFrame = length(roiData{1}.imageData{1}{1}{1}); % total number of frames in data set
...
for channel = 1:chans
    ...
    for strip = 1:numROIs
        ...
        for frame = 1:totalFrame
            stripTemp = cat(4,stripTemp, roiData{1,strip}.imageData{1,channel}{1,1}{1,1}{frame});
        end
        corr = returnScanOffset(stripTemp,1, 'int16'); % find offset correction
        stripTemp = fixScanPhase(stripTemp,corr,1, 'int16'); % fix scan phase
    ...
    imageData = cat(3,imageData,frameTemp);
    fprintf("Channel %d finished in %.2f.\n... Total time: %.2f", channel, toc(tchannel). toc(tstart))
end
end
