...
spath = '\\v-storage.rockefeller.edu\vazirilab_medium_data\jeff_demas\MAxiMuM_data\Paper_Figures\NewFig3\';
...
figure;
hold on
h = fill([tt fliplr(tt)],[mean(btrials(inup,:,:),3)+std(btrials(inup,:,:),[],3) fliplr(mean(btrials(inup,:,:),3)-std(btrials(inup,:,:),[],3))],[1 0.5 0],'LineStyle','none');
h.FaceAlpha = falpha;
...
h.FaceAlpha = falpha;
plot(tt,mean(btrials(inup,:,:),3),'m-','Color',[1 0.5 0])
...
figure;
hold on
h = fill([tt fliplr(tt)],[mean(btrials(indn,:,:),3)+std(btrials(indn,:,:),[],3) fliplr(mean(btrials(indn,:,:),3)-std(btrials(indn,:,:),[],3))],[1 0.5 0],'LineStyle','none');
h.FaceAlpha = falpha;
...
h.FaceAlpha = falpha;
plot(tt,mean(btrials(indn,:,:),3),'m-','Color',[1 0.5 0])
...
hold on
% h.FaceAlpha = 0.6;
h=histogram(100.*schange(sigoi&upmod),-200:5:200,'FaceColor',[1 0.5 0]);
h.FaceAlpha = 0.6;
...
scolors(schangec>0,:) = ones(sum(logical(schangec>0)),1)*[1 0.5 0];
scolors(schangec<0,:) = ones(sum(logical(schangec<0)),1)*[0 1 1];
...
figure;
[psrt,inpsrt] = sort(p,'ascend');
for j = 1:6
    inoi = inpsrt(j);

    vtoi = reshape(btrials(inoi,:,:),size(btrials,2),[])';
    smeanoi = mean(strials(inoi,:,:),3);
    sstdoi = std(strials(inoi,:,:),[],3);

    rms = sqrt(mean(abs(vtoi - ones(size(vtoi,1),1)*smeanoi).^2,2));
    [rmssrt,inrmssrt] = sort(rms,'ascend');
    indsoi = round(linspace(1,numel(rmssrt),9));

    subplot(6,10,(j-1)*10+1)
    hold on
    h = fill([tt fliplr(tt)],[mean(strials(inoi,:,:),3)+std(strials(inoi,:,:),[],3) fliplr(mean(strials(inoi,:,:),3)-std(strials(inoi,:,:),[],3))],'c','LineStyle','none');
    h.FaceAlpha = falpha;
    h = fill([tt fliplr(tt)],[mean(btrials(inoi,:,:),3)+std(btrials(inoi,:,:),[],3) fliplr(mean(btrials(inoi,:,:),3)-std(btrials(inoi,:,:),[],3))],[1 0.5 0],'LineStyle','none');
    h.FaceAlpha = falpha;
    plot(tt,mean(btrials(inoi,:,:),3),'m-','Color',[1 0.5 0])
    plot(tt,mean(strials(inoi,:,:),3),'c-','Color',[0 0.6 0.9])
    box(gca,'on')
    xlim([-1 3])
    % axis off
    set(gca,'XColor',[0 0 0],'YColor',[ 0 0 0],'LineWidth',1)
    ylim([-0.2 2.8])

    for k = 1:9
        subplot(6,10,(j-1)*10+1+k)
        plot(tt,vtoi(inrmssrt(indsoi(k)),:),'k-')
        ylim([-0.2 2.8]); xlim([-1 3])
        set(gca,'XColor',[0 0 0],'YColor',[ 0 0 0],'LineWidth',1)
    %     axis off
    end
end

set(gcf,'Position',[10 50 1250 650])
saveas(gcf,[spath 'wsk_mod_vis_trials.fig'])
print(gcf,[spath 'wsk_mod_vis_trials.png'],'-dpng','-r600')
...
hold on
% h.FaceAlpha = 0.6;
h=histogram(100.*vchange(sigoi&upmod),-200:5:200,'FaceColor',[1 0.5 0]);
h.FaceAlpha = 0.6;
...
vcolors(vchangec>0,:) = ones(sum(logical(vchangec>0)),1)*[1 0.5 0];
vcolors(vchangec<0,:) = ones(sum(logical(vchangec<0)),1)*[0 1 1];
...
figure;
[~,inpsrt] = sort(p,'ascend');
for j = 1:6
    inoi = inpsrt(j);

    stoi = reshape(btrials(inoi,:,:),size(btrials,2),[])';
    vmeanoi = mean(vtrials(inoi,:,:),3);
    sstdoi = std(vtrials(inoi,:,:),[],3);

    rms = sqrt(mean(abs(stoi - ones(size(stoi,1),1)*vmeanoi).^2,2));
    [rmssrt,inrmssrt] = sort(rms,'ascend');
    indsoi = round(linspace(1,numel(rmssrt),9));

    subplot(6,10,(j-1)*10+1)
    hold on
    h = fill([tt fliplr(tt)],[mean(vtrials(inoi,:,:),3)+std(vtrials(inoi,:,:),[],3) fliplr(mean(vtrials(inoi,:,:),3)-std(vtrials(inoi,:,:),[],3))],'c','LineStyle','none');
    h.FaceAlpha = falpha;
    h = fill([tt fliplr(tt)],[mean(btrials(inoi,:,:),3)+std(btrials(inoi,:,:),[],3) fliplr(mean(btrials(inoi,:,:),3)-std(btrials(inoi,:,:),[],3))],[1 0.5 0],'LineStyle','none');
    h.FaceAlpha = falpha;
    plot(tt,mean(btrials(inoi,:,:),3),'m-','Color',[1 0.5 0])
    plot(tt,mean(vtrials(inoi,:,:),3),'c-','Color',[0 0.6 0.9])
    box(gca,'on')
    xlim([-1 3])
    % axis off
    set(gca,'XColor',[0 0 0],'YColor',[ 0 0 0],'LineWidth',1)
    ...
    for k = 1:9
        subplot(6,10,(j-1)*10+1+k)
        plot(tt,stoi(inrmssrt(indsoi(k)),:),'k-')
        ylim([-0.2 2.8]); xlim([-1 3])
        set(gca,'XColor',[0 0 0],'YColor',[ 0 0 0],'LineWidth',1)
    %     axis off
    end
...
set(gcf,'Position',[10 50 1250 650])
saveas(gcf,[spath 'vis_mod_wsk_trials.fig'])
print(gcf,[spath 'vis_mod_wsk_trials.png'],'-dpng','-r600')
...
h.FaceAlpha = falpha;
plot(tt,mean(strialsmo(inup,:,:),3),'m-','Color',[1 0.5 0])
...
figure;
hold on
h = fill([tt fliplr(tt)],[mean(strialsmo(indn,:,:),3)+std(strialsmo(indn,:,:),[],3) fliplr(mean(strialsmo(indn,:,:),3)-std(strialsmo(indn,:,:),[],3))],[1 0.5 0],'LineStyle','none');
h.FaceAlpha = falpha;
...
h.FaceAlpha = falpha;
plot(tt,mean(strialsmo(indn,:,:),3),'m-','Color',[1 0.5 0])
...
hold on
% h.FaceAlpha = 0.6;
h=histogram(100.*smchange(sigoi&upmod),-200:5:200,'FaceColor',[1 0.5 0]);
h.FaceAlpha = 0.6;
...
smcolors(smchangec>0,:) = ones(sum(logical(smchangec>0)),1)*[1 0.5 0];
smcolors(smchangec<0,:) = ones(sum(logical(smchangec<0)),1)*[0 1 1];
...
for j = 1:6
    ...
    toi = reshape(strialsmo(inoi,:,:),size(strialsmo,2),[])';
    meanoi = mean(strialsnmo(inoi,:,:),3);
    stdoi = std(strialsnmo(inoi,:,:),[],3);

    rms = sqrt(mean(abs(toi - ones(size(toi,1),1)*meanoi).^2,2));
    [rmssrt,inrmssrt] = sort(rms,'ascend');
    indsoi = round(linspace(1,numel(rmssrt),9));

    subplot(6,10,(j-1)*10+1)
    hold on
    h = fill([tt fliplr(tt)],[mean(strialsnmo(inoi,:,:),3)+std(strialsnmo(inoi,:,:),[],3) fliplr(mean(strialsnmo(inoi,:,:),3)-std(strialsnmo(inoi,:,:),[],3))],'c','LineStyle','none');
    h.FaceAlpha = falpha;
    h = fill([tt fliplr(tt)],[mean(strialsmo(inoi,:,:),3)+std(strialsmo(inoi,:,:),[],3) fliplr(mean(strialsmo(inoi,:,:),3)-std(strialsmo(inoi,:,:),[],3))],[1 0.5 0],'LineStyle','none');
    h.FaceAlpha = falpha;
    plot(tt,mean(strialsmo(inoi,:,:),3),'m-','Color',[1 0.5 0])
    plot(tt,mean(strialsnmo(inoi,:,:),3),'c-','Color',[0 0.6 0.9])
    box(gca,'on')
    xlim([-1 3])
    % axis off
    set(gca,'XColor',[0 0 0],'YColor',[ 0 0 0],'LineWidth',1)
    ...
    for k = 1:9
        subplot(6,10,(j-1)*10+1+k)
        plot(tt,toi(inrmssrt(indsoi(k)),:),'k-')
        ylim([-0.2 5]); xlim([-1 3])
        set(gca,'XColor',[0 0 0],'YColor',[ 0 0 0],'LineWidth',1)
    end
...
set(gcf,'Position',[10 50 1250 650])
saveas(gcf,[spath 'wsk_mod_motion_trials.fig'])
print(gcf,[spath 'wsk_mod_motion_trials.png'],'-dpng','-r600')
...
figure;
hold on
h = fill([tt fliplr(tt)],[mean(vtrialsmo(inup,:,:),3)+std(vtrialsmo(inup,:,:),[],3) fliplr(mean(vtrialsmo(inup,:,:),3)-std(vtrialsmo(inup,:,:),[],3))],[1 0.5 0],'LineStyle','none');
h.FaceAlpha = falpha;
...
h.FaceAlpha = falpha;
plot(tt,mean(vtrialsmo(inup,:,:),3),'m-','Color',[1 0.5 0])
...
figure;
hold on
h = fill([tt fliplr(tt)],[mean(vtrialsmo(indn,:,:),3)+std(vtrialsmo(indn,:,:),[],3) fliplr(mean(vtrialsmo(indn,:,:),3)-std(vtrialsmo(indn,:,:),[],3))],[1 0.5 0],'LineStyle','none');
h.FaceAlpha = falpha;
...
h.FaceAlpha = falpha;
plot(tt,mean(vtrialsmo(indn,:,:),3),'m-','Color',[1 0.5 0])
...
hold on
% h.FaceAlpha = 0.6;
h=histogram(100.*vmchange(sigoi&upmod),-200:5:200,'FaceColor',[1 0.5 0]);
h.FaceAlpha = 0.6;
...
vmcolors(vmchangec>0,:) = ones(sum(logical(vmchangec>0)),1)*[1 0.5 0];
vmcolors(vmchangec<0,:) = ones(sum(logical(vmchangec<0)),1)*[0 1 1];
...
for j = 1:6
    ...
    toi = reshape(vtrialsmo(inoi,:,:),size(vtrialsmo,2),[])';
    meanoi = mean(vtrialsnmo(inoi,:,:),3);
    stdoi = std(vtrialsnmo(inoi,:,:),[],3);
    ...
    subplot(6,10,(j-1)*10+1)
    hold on
    h = fill([tt fliplr(tt)],[mean(vtrialsnmo(inoi,:,:),3)+std(vtrialsnmo(inoi,:,:),[],3) fliplr(mean(vtrialsnmo(inoi,:,:),3)-std(vtrialsnmo(inoi,:,:),[],3))],'c','LineStyle','none');
    h.FaceAlpha = falpha;
    h = fill([tt fliplr(tt)],[mean(vtrialsmo(inoi,:,:),3)+std(vtrialsmo(inoi,:,:),[],3) fliplr(mean(vtrialsmo(inoi,:,:),3)-std(vtrialsmo(inoi,:,:),[],3))],[1 0.5 0],'LineStyle','none');
    h.FaceAlpha = falpha;
    plot(tt,mean(vtrialsmo(inoi,:,:),3),'m-','Color',[1 0.5 0])
    plot(tt,mean(vtrialsnmo(inoi,:,:),3),'c-','Color',[0 0.6 0.9])
    box(gca,'on')
    xlim([-1 3])
    % axis off
    set(gca,'XColor',[0 0 0],'YColor',[ 0 0 0],'LineWidth',1)
...
set(gcf,'Position',[10 50 1250 650])
saveas(gcf,[spath 'vis_mod_motion_trials.fig'])
print(gcf,[spath 'vis_mod_motion_trials.png'],'-dpng','-r600')