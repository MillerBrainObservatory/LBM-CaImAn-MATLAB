...
path = '//vmd/jeff_demas/MAxiMuM_data/20191122/mh89_hemisphere_FOV_50_550um_depth_250mW_dual_stimuli_30min_00001/';
...
addpath(genpath('//vmd/jeff_demas/PROCESSING_SCRIPTS/Segmentation_Routines/CaImAn-MATLAB-master/CaImAn-MATLAB-master/'))
...
spath = '/home/jdemas/'; % saving path
fileName = [spath 'MAxiMuM_hemisphere_movie_upsampled.avi']; % saving file

writerObj = VideoWriter(fileName,'Uncompressed AVI'); % create AVI object
writerObj.FrameRate = frameRate; % 4x speed up
...
T = 564;
t = (1:564)./564.*120;
Tp = round(120/speedup*frameRate);
tp = (1:Tp)./Tp.*120;

tracesp = zeros(size(traces,1),Tp);

K = size(traces,1);
for k = 1:K
    tracesp(k,:) = interp1(t,traces(k,:),tp);
end
...
ssk = interp1(t,ssk,tp); ssk(:,1) = ssk(:,2);

svk = movmean(Sv(1:downsampling:564),averaging);
svk = interp1(t,svk,tp); svk(:,1) = svk(:,2);
